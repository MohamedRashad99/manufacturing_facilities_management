...
class ProductionDeliveryOrderGrid extends StatelessWidget {
  const ProductionDeliveryOrderGrid({super.key, required this.offstage});
  final bool offstage;
  ...
  Widget build(BuildContext context) {
    final count =
        context.read<ProductionDeliveryOrderCubit>().rowsOnyxIx.length + 2.7;
    return SizedBox(
      height:
          count *
          rowHeight, // 16 length for grid cell + 2.5 for border && header && footer = 18.5
      child: OnyxIxGrid(
        columns: context.read<ProductionDeliveryOrderCubit>().columnsOnyxIx,
        rows: context.read<ProductionDeliveryOrderCubit>().rowsOnyxIx,
        onLoaded: context.read<ProductionDeliveryOrderCubit>().onLoadedOnyxIx,
